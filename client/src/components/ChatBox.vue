<template>
    <div class="h-100">
        <div class="chatbox-container row no-gutters flex-column" v-if="getChatView">
            <div class="profile col-auto">
                <img class="image" src="http://soho.laborasyon.com/default/dist/media/img/man_avatar3.jpg" />
                <div class="detail">
                    <p class="name">{{getFriendNumber(getChatView.rooms.participant)}}</p>
                    <small class="status">Online</small>
                </div>
            </div>
            <div class="chats col pr-4" id="chats-container">
                <div class="chat" :class="list.user === getProfile._id ? 'active':''" v-for="list in getChatView.chats" v-bind:key="list">
            ...
            <div class="chat-search col-auto">
                <form>
                    <input type="text" class="form-control" v-model="message" placeholder="Say hi.." />
                    <input type="submit" class="d-none" v-on:click="sendMessage" />
                </form>
            </div>
        </div>
        <div class="chatbox-container h5 mb-0 font-weight-bold row no-gutters flex-column align-items-center justify-content-center" v-else>
